...
MAX31856::MAX31856(spi_host_device_t aHostId, spi_bus_config_t aBusConfig): myHostId(aHostId)
{
...
void MAX31856::AddDevice(ThermocoupleType aType, gpio_num_t aCsPin, uint8_t anIndex)
{
    ...
    this->myThermocoupleType[anIndex] = aType;
...
uint8_t MAX31856::readFault(bool log_fault, uint8_t anIndex)
{
	...
	myLastResult[anIndex].fault = fault_val;
...
float MAX31856::readColdJunction(uint8_t anIndex)
{
	oneshotTemperature(anIndex);
	uint16_t cj_temp = readRegister16(MAX31856_CJTH_REG, anIndex);
	float cj_temp_float = cj_temp;
	cj_temp_float /= 256.0;
	myLastResult[anIndex].coldjunction_c = cj_temp_float;
	myLastResult[anIndex].coldjunction_f = (1.8 * cj_temp_float) + 32.0;
	return cj_temp_float;
}

float MAX31856::readTemperature(uint8_t anIndex)
{
	oneshotTemperature();
	...
	myLastResult[anIndex].thermocouple_c = tc_temp_float;
	myLastResult[anIndex].thermocouple_f = (1.8 * tc_temp_float) + 32.0;
	return tc_temp_float;
}

void MAX31856::setType(ThermocoupleType type, uint8_t anIndex)
{
	myThermocoupleType[anIndex] = type;
